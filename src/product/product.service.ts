...
export class ProductService {
  ...
  async getProductDetails(id: string, locationId: string, token: string): Promise<any> {
    try {
      const response = await axios.get(
        `https://api.kroger.com/v1/products/${id}?filter.locationId=${locationId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: 'application/json',
